...
This repository is not just the bare framework template. It already includes an initialized project:

- `research/autoresearch/projects/memory-policy/`

That project was bootstrapped through the framework flow and then populated with the current memory-policy research materials.

The repository also preserves prior runtime evidence under:

- `.autoresearch/projects/memory-policy/`

That includes accepted and rejected run records, health reports, dashboard artifacts, sentinel phase state, and prior candidate/evaluation outputs.
